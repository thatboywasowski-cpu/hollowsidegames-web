-- Hollowside downloads and comment repair v10
-- Run this in Supabase after v9. It restores comments, adds the Downloads
-- permission, and creates the Downloads catalog used by /downloads.
...
begin
    ...
    if public.is_account_banned(v_actor_id) or public.is_account_suspended(v_actor_id) then
        raise exception 'Restricted accounts cannot publish downloads.';
    end if;
    ...
    if char_length(trim(coalesce(p_download_url, ''))) = 0 then
        raise exception 'Downloads must include a URL.';
    end if;
